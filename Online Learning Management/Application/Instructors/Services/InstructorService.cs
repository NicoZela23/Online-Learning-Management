...
namespace Online_Learning_Management.Application.Instructors.Services
{
    public class InstructorService : INstructorService
    {
        ...
        public async Task<IEnumerable<Course>> GetCoursesByIdInstructorAsync(Guid id)
        {
            var instructor = await _instructorRepository.GetInstructorByIdAsync(id);
            if(instructor == null)
            {
            ...
            var courses = await _instructorRepository.GetCoursesByInstructorIdAsync(id);
            if(courses == null)
            {
                throw new ArgumentException("There are no courses for this instructor.");
            }
